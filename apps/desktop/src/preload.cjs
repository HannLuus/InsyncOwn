...
contextBridge.exposeInMainWorld("insyncown", {
  ...
  getNetworkSettings: () => ipcRenderer.invoke("daemon:getNetworkSettings"),
  setNetworkSettings: (input) => ipcRenderer.invoke("daemon:setNetworkSettings", input),
  testNetwork: () => ipcRenderer.invoke("daemon:testNetwork"),
